...
def parser_args():
    ...
    parser.add_argument('--data_path', type=str,
                        default='./module/datasets/dev_CoNLL2003.json')
    ...
    parser.add_argument('--language_ckpt_path', type=str,
                        default='./basetest/cache/bert-large-cased')  # the path that stores the imagebind checkpoint
    ...
    parser.add_argument('--delta_ckpt_path', type=str,
                        default='./ckpt/pretrained_ckpt')  # the delta parameters trained in stage 1
...
def initialize_distributed(args):
    ...
    args['master_port'] = os.getenv('MASTER_PORT', '28457')
...
def main(**args):
    ...
    test_data, test_iter, sampler = load_sft_dataset(args, args['test_data_path'])

    length = args['epochs'] * len(test_data) // args['world_size'] // dschf.config[
        'train_micro_batch_size_per_gpu']
    total_steps = args['epochs'] * len(test_data) // dschf.config['train_batch_size']
    ...
    model = agent.model.eval().half().cuda()
